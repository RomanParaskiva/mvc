...
class Calendar
{
     ...
     * @param int $month
     * @param int $year 
     * @param array $events Массив отмеченных дат
     */
    public static function getMonth($month, $year, $events = [])
    {
        ...
        for ($day = 1; $day <= $days_month; $day++) {
            ...
            if ($event_show) {
                $out .= '<td class="calendar-day ' . $class . ' event"><a href="' . Utils::generateLink('calendar/day/' . strtotime($day . '.' . $month . '.' . $year)) . '">' . $day; 
                if (!empty($event_text)) {
                    $out .=
                        '<div class="calendar-popup">' .
                        implode('<br>', $event_text) .
                        '</div>';
                }
                $out .= '</a></td>';
            } else {
                $out .=
                    '<td class="calendar-day ' . $class . '"><a href="' . Utils::generateLink('calendar/day/' . strtotime($day . '.' . $month . '.' . $year)). '">' . $day . '</a></td>';
            }
